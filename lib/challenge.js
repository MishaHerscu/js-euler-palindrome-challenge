...
const check = function(number){
  ...
  let baseEight = number.toString(8).split('');

  // check baseTwo
  let baseTwoPalindrome = checkString(baseTwo);
  let baseTenPalindrome = checkString(baseTen);
  let baseEightPalindrome = checkString(baseEight);

  if(baseTenPalindrome === true && baseTwoPalindrome === true && baseEightPalindrome === true){
    return true;
  } else{
    return false;
  }
